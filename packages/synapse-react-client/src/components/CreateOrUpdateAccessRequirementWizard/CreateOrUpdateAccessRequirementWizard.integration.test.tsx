...
describe('CreateOrUpdateAccessRequirementWizard', () => {
  ...
  test('updates ACT AR', async () => {
    ...
    // step 2: basic AR fields
    const deleteTextInstructions = await screen.findByRole('button', {
      name: 'Delete old instructions',
    })
    await user.click(deleteTextInstructions)
    ...
    expect(deleteTextInstructions).not.toBeInTheDocument()

    await user.click(getBtn('Save'))

    await waitFor(() => {
      expect(updateAccessRequirementSpy).toHaveBeenCalledTimes(3)
      expect(updateAccessRequirementSpy).toHaveBeenLastCalledWith(
        MOCK_ACCESS_TOKEN,
        {
          ...mockACTAccessRequirement,
