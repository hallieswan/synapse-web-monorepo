...
export type TeamSubjectsSelectorProps = {
  ...
  onUpdateTeamIDsTextbox?: (value: string) => void // when the team IDs text box is updated, this is called
}
...
> = (props: TeamSubjectsSelectorProps) => {
  ...
  const [teamIdsString, setTeamIdsString] = useState<string>('')
  const [error, setError] = useState<string | null>(null)

  const teamSubjects = subjects.filter(subject => {
    ...
    if (teamIds && teamIds.length > 0) {
      ...
      for (const newTeamId of teamIds) {
        ...
        if (alreadyAdded) {
          ...
          newTeamSubjects.push(newSubject)
        }
      }
      onChangeTeamIdsTextbox('')
      onUpdate(newTeamSubjects)
    }
    setError(null)
  }
